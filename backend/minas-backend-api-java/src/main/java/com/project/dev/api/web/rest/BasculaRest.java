...
import com.project.dev.api.web.rest.assembler.BasculaRestAssembler;
...
/**
 * TODO: Definición de {@code BasculaRest}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
@Api(value = "Bascula API")
public class BasculaRest {

    private final Logger log = LoggerFactory.getLogger(BasculaRest.class);
    private final BasculaService entityService;
    private final BasculaRestAssembler entityRestAssembler;
     ...
     */
    public BasculaRest(BasculaService entityService,
            BasculaRestAssembler entityRestAssembler) {
    ...
    public CollectionModel<EntityModel<BasculaDTO>> getAllEntities() {
        ...
        try {
            ...
            CollectionModel<EntityModel<BasculaDTO>> entitiesCollection = new CollectionModel<>(entities);
            entitiesCollection.add(linkTo(methodOn(BasculaRest.class).getAllEntities()).withSelfRel());
    ...
    public ResponseEntity<List<BasculaDTO>> getAllEntitiesPaged(Pageable pageable) {
    ...
    @GetMapping("/Bascula/{query}/pages")
    public ResponseEntity<List<BasculaDTO>> searchEntitiesPaged(@PathVariable String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/Bascula/{query}/pages/" + query);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
