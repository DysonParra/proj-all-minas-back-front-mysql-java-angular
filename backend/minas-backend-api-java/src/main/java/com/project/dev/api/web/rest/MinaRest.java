...
import com.project.dev.api.web.rest.assembler.MinaRestAssembler;
...
/**
 * TODO: Definición de {@code MinaRest}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
@Api(value = "Mina API")
public class MinaRest {

    private final Logger log = LoggerFactory.getLogger(MinaRest.class);
    private final MinaService entityService;
    private final MinaRestAssembler entityRestAssembler;
     ...
     */
    public MinaRest(MinaService entityService,
            MinaRestAssembler entityRestAssembler) {
    ...
    public CollectionModel<EntityModel<MinaDTO>> getAllEntities() {
        ...
        try {
            ...
            CollectionModel<EntityModel<MinaDTO>> entitiesCollection = new CollectionModel<>(entities);
            entitiesCollection.add(linkTo(methodOn(MinaRest.class).getAllEntities()).withSelfRel());
    ...
    public ResponseEntity<List<MinaDTO>> getAllEntitiesPaged(Pageable pageable) {
    ...
    @GetMapping("/Mina/{query}/pages")
    public ResponseEntity<List<MinaDTO>> searchEntitiesPaged(@PathVariable String query, Pageable pageable) {
        ...
        HttpHeaders headers = PaginationUtil.generatePaginationHttpHeaders(page, "/api/Mina/{query}/pages/" + query);
        return new ResponseEntity<>(page.getContent(), headers, HttpStatus.OK);
    }
