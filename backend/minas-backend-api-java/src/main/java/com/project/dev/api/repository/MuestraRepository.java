/*
 * @fileoverview    {MuestraRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code MuestraRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface MuestraRepository extends JpaRepository<Muestra, Integer> {
     ...
     */
    public List<Muestra> findByIntIdMuestra(Integer id);
    ...
    @Query("SELECT m FROM Muestra m WHERE m.intIdMuestra LIKE CONCAT('%', :strSearch, '%')")
    public List<Muestra> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Muestra m WHERE m.intIdMuestra LIKE CONCAT('%', :strSearch, '%')")
    public Page<Muestra> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
