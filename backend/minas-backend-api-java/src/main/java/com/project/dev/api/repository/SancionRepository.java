...
/**
 * TODO: Definición de {@code SancionRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface SancionRepository extends JpaRepository<Sancion, Integer> {
     ...
     */
    public List<Sancion> findByIntNumero(Integer id);
    ...
    @Query("SELECT m FROM Sancion m WHERE m.intNumero LIKE CONCAT('%', :strSearch, '%')")
    public List<Sancion> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Sancion m WHERE m.intNumero LIKE CONCAT('%', :strSearch, '%')")
    public Page<Sancion> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
