/*
 * @fileoverview    {ProductoRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ProductoRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ProductoRepository extends JpaRepository<Producto, Integer> {
     ...
     */
    public List<Producto> findByIntIdProducto(Integer id);
    ...
    @Query("SELECT m FROM Producto m WHERE m.intIdProducto LIKE CONCAT('%', :strSearch, '%')")
    public List<Producto> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Producto m WHERE m.intIdProducto LIKE CONCAT('%', :strSearch, '%')")
    public Page<Producto> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
