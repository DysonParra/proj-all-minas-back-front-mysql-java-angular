...
/**
 * TODO: Definición de {@code ControlAccesoRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ControlAccesoRepository extends JpaRepository<ControlAcceso, Integer> {
     ...
     */
    public List<ControlAcceso> findByIntIdControl(Integer id);
    ...
    @Query("SELECT m FROM ControlAcceso m WHERE m.intIdControl LIKE CONCAT('%', :strSearch, '%')")
    public List<ControlAcceso> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM ControlAcceso m WHERE m.intIdControl LIKE CONCAT('%', :strSearch, '%')")
    public Page<ControlAcceso> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
