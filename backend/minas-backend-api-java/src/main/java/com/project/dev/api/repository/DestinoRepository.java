/*
 * @fileoverview    {DestinoRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code DestinoRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface DestinoRepository extends JpaRepository<Destino, String> {
     ...
     */
    public List<Destino> findByStrIdDestino(String id);
    ...
    @Query("SELECT m FROM Destino m WHERE m.strIdDestino LIKE CONCAT('%', :strSearch, '%')")
    public List<Destino> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Destino m WHERE m.strIdDestino LIKE CONCAT('%', :strSearch, '%')")
    public Page<Destino> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
