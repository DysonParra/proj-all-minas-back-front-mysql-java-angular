/*
 * @fileoverview    {ProveedorRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ProveedorRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ProveedorRepository extends JpaRepository<Proveedor, String> {
     ...
     */
    public List<Proveedor> findByStrCif(String id);
    ...
    @Query("SELECT m FROM Proveedor m WHERE m.strCif LIKE CONCAT('%', :strSearch, '%')")
    public List<Proveedor> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Proveedor m WHERE m.strCif LIKE CONCAT('%', :strSearch, '%')")
    public Page<Proveedor> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
