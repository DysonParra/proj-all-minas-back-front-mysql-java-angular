/*
 * @fileoverview    {ParqueRepository} se encarga de realizar tareas específicas.
 *
 * @version         2.0
 *
...
/**
 * TODO: Definición de {@code ParqueRepository}.
 *
 * @author Dyson Parra
 * @since 1.8
 */
...
public interface ParqueRepository extends JpaRepository<Parque, Integer> {
     ...
     */
    public List<Parque> findByIntIdParque(Integer id);
    ...
    @Query("SELECT m FROM Parque m WHERE m.intIdParque LIKE CONCAT('%', :strSearch, '%')")
    public List<Parque> searchEntities(@Param("strSearch") String strSearch);
    ...
    // TODO: Especificar campo de la entidad donde realizar la búsqueda.
    @Query("SELECT m FROM Parque m WHERE m.intIdParque LIKE CONCAT('%', :strSearch, '%')")
    public Page<Parque> searchEntities(@Param("strSearch") String strSearch, Pageable pageable);
}
